...
const lapseleaveRouter = express.Router();
...
// ---------------seperate leave section with lapse option-----------------

// -----------leave lapse----------------

//all leaves balance
lapseleaveRouter.get('/', async (req, res) => {
  const lapaseleave = await LeaveLapse.findAll();
  if (lapaseleave) {
    // Send the created employees as the response
    return res.status(200).send({ lapaseleave });
  } else {
...
lapseleaveRouter.get('/:employeeid', async (req, res) => {
  ...
  if (lapaseleave) {
    ...
    return res.status(400).send({ message: 'Leave Not found1' });
  }
});
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (!employee) {
        return res
          .status(404)
          .send({ message: 'Leaves Not Found' + employeeId });
      }
      ...
      if (lapaseleave[leaveType] >= numberOfDays) {
        ...
        transporter.sendMail(
          ...
          margin: 5vh auto;
              ...
              leaveApplication.employee_id
                    ...
                    <a
                    href='${baseUrl()}/leave-application/${
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    try {
      ...
      switch (leaveEntry.type) {
        case 'casual':
          lapaseleave.casual -= numberOfDays;
          break;
        case 'privilege':
          lapaseleave.privilege -= numberOfDays;
          break;
        case 'sick':
          lapaseleave.sick -= numberOfDays;
          break;
      ...
      lapaseleave.leaves -= numberOfDays;
...
lapseleaveRouter.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    try {
      ...
      // Check if the leave is already rejected
      if (leaveEntry.approved === '0') {
      ...
      // Deduct the leaves if the leave was approved
      if (leaveEntry.approved === '1') {
        // Find the leave lapse entry for the employee
        let lapaseleave = await LeaveLapse.findOne({
          where: { employee_id: employeeid }, // Ensure employee_id is defined
        });

        // Deduct leaves from the leave lapse entry based on leave type
        switch (leaveEntry.type) {
          case 'casual':
            lapaseleave.casual -= numberOfDays;
            break;
        ...
        // Save the updated leave lapse entry
        await lapaseleave.save();
      }
...
export default lapseleaveRouter;
