...
function SalarySlip() {
  ...
  const [{ loading, error, payslip }, dispatch] = useReducer(reducer, {
    payslip: {},
    loading: true,
    error: '',
  });
  ...
  useEffect(() => {
    ...
    const fetchData2 = async () => {
      ...
      try {
        const result = await axios.get(`/api/payslip/${id}/${month}/${year}`);
        dispatch({ type: 'FETCH_SUCCESS', payload: result.data.payslip });
        const employeeresult = await axios.get(`/api/employees/${id}`);
  ...
  const generatePdf = (employee, year, month) => {
    ...
    doc.text(`${payslip.employee_id}`, 47, 47);
    //last vertical line
    ...
    doc.setTextColor(0); // Reset text color to black
    doc.text(`${payslip.designation}`, 152, 40);
    ...
    doc.setTextColor(0); // Reset text color to black
    doc.text(`${payslip.pf_account_no}`, 248, 40);

    doc.setTextColor(0, 135, 255); // Set text color to blue
    doc.text(`UAN No.`, 230, 47);

    doc.setTextColor(0); // Reset text color to black
    doc.text(`${payslip.uan_number}`, 248, 47);
    //vertical line
    ...
    doc.text(
      `Fixed payment that employers provide to payslip to cover their medical expenses`,
      122,
      89
    );
    ...
    const netSalary =
      (totaldays * (payslip.ctc / 12)) / numberOfDaysInCurrentMonth -
    ...
    // const BASIC = payslip.;
    const BASIC = ((payslip.ctc / 12) * (45 / 100)).toFixed(2);
    ...
    // old code
    // const totalPaidLeaveEntitlement = payslip.leaves;
    ...
    // Check if isProbation is 1
    const isProbation = payslip.isProbation === 0;

    // Calculate PaidLeaveTaken based on the isProbation condition
    const PaidLeaveTaken = isProbation
      ? 0
      : Math.max(0, totalLeavesAvailable - totalPaidLeaveEntitlement);
  ...
  const generateAndPreviewPdf = () => {
    const employeeId = id;
    const salarySlipPdf = generatePdf(payslip, year, month, totaldays);
    if (salarySlipPdf) {
  ...
  useEffect(() => {
    ...
    generateAndPreviewPdf(payslip, month, totaldays);
  });

  const generateAndDownloadPdf = () => {
    const salarySlipPdf = generatePdf(payslip, year, month);
    if (salarySlipPdf) {
      salarySlipPdf.save(`${payslip.NAME}_${month}_${year}.pdf`);
    } else {
      console.log('PDF generation failed.');
    }
  };
